...
// get Date from user.php for resetting password
@$seller_id = $_GET['e5HxuYol1Io8JLdit'];
@$company_title = $_GET['c5HxuYol1Io8JL_i'];
@$token_key = $_GET['token'];
// fetch seller info

$sqq = mysqli_query($conn, "SELECT * FROM seller_request_table JOIN seller_profile_table ON seller_request_table.seller_id = seller_profile_table.seller_req_id WHERE seller_request_table.seller_id = '$seller_id' AND seller_request_table.seller_company_title = '$company_title' ");
...
if ($pass != $pass1 AND $password != $password1) {
...
{
  $qq_up =" UPDATE seller_profile_table JOIN seller_request_table ON seller_profile_table.seller_req_id = seller_request_table.seller_id SET seller_password = '$password' WHERE seller_id = '$seller_id' ";
  if(!mysqli_query($conn, $qq_up))
  {
$err = "Query Error";
}
else
...
<div class="d-inline">
<h5>Seller: <?php echo $rw['seller_company_title']; ?></h5>
...
<h5>Seller ID: <?php echo $rw['seller_id']; ?></h5>
...
$token = "ya29.5HxuYol1Io8JLeGePDznbfkkwu_PC4uodKwG8_1clFYAn9AgdOV1WGpOTNQP3s76HAsn7Y4zWw";
if (@$token_key === $token AND @$rw['seller_id'] = $seller_id AND @$rw['seller_company_title'] == $company_title ) {
 ?>

<div class="container">
<form method="POST"> 

 <input type="password" class="form-control m-3" name="pass" placeholder="Type Password">
 <input type="password" class="form-control m-3" name="pass1" placeholder="Confirm Password">
<br>
<button type="submit" name="update" class="btn btn-danger btn-sm waves-effect text-center m-b-20 iinntt" style="background-color: #dc3545;margin-left: 15px;">Reset User Password</button>
...
?>
<div class="container">
	<p style="color:#dc3545; ">Url Error</p>
</div>
<?php	
}
